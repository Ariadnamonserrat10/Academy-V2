...
import { FiUser, FiShield, FiCheckCircle, FiAlertCircle, FiDroplet, FiZap, FiImage, FiLock, FiSmartphone, FiSkipForward } from 'react-icons/fi'
import { useWeb3 } from '../context/Web3Context'
import { useAuth } from '../context/AuthContext'
import { fundTestnetAccount } from '../utils/freighter'
import { authenticateWithPasskey, PasskeyCancelled, requireBiometric } from '../utils/passkeyAuth'
import { logAudit, isAccountLocked, getRemainingAttempts } from '../utils/auditLog'

export default function Login() {
  const { address, isConnected, connect, isConnecting, error, clearError } = useWeb3()
  const { login, loginTeacher, accountExists, verifyNftImage, hasPasskey } = useAuth()
  ...
  const requireNftBiometric = async (action: string): Promise<boolean> => {
    ...
    try {
      ...
      setNftBiometricError('Error de verificación biométrica')
      return false
    } finally {
  ...
  const handlePasskeyAuth = async (): Promise<boolean> => {
    setPasskeyLoading(true)
    setLoginError('')
    try {
      await authenticateWithPasskey()
      setPasskeyVerified(true)
      logAudit('PASSKEY_AUTH', { username })
      return true
    } catch (e: any) {
      if (e instanceof PasskeyCancelled) {
        logAudit('PASSKEY_SKIPPED', { username })
        return false
      }
      setLoginError('Verificación biométrica fallida')
      logAudit('PASSKEY_FAILED', { username })
      return false
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    if (step === 2) {
      ...
      if (!passkeyVerified && hasPasskey(username.trim())) {
        const ok = await handlePasskeyAuth()
        if (!ok) return
      }
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              {hasPasskey(username.trim()) && (
                <div className="border border-slate-200 dark:border-slate-600 rounded-xl p-3">
                  <h5 className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1">
                    <FiSmartphone className="w-3 h-3" /> Seguridad biométrica
                  </h5>
                  {passkeyVerified ? (
                    <div className="p-2 bg-green-50 dark:bg-green-900/20 border border-green-200 rounded-lg text-xs text-green-600 flex items-center justify-center gap-1">
                      <FiLock className="w-3 h-3" /> Biometría verificada
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <button onClick={handlePasskeyAuth} disabled={passkeyLoading} className="flex-1 py-2 text-sm bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 text-blue-600 dark:text-blue-400 rounded-lg font-medium hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all disabled:opacity-50 flex items-center justify-center gap-1.5">
                        <FiLock className="w-3.5 h-3.5" /> {passkeyLoading ? 'Verificando...' : 'Windows Hello'}
                      </button>
                      <button onClick={() => setPasskeyVerified(false)} className="px-3 py-2 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors flex items-center gap-1">
                        <FiSkipForward className="w-3 h-3" /> Saltar
                      </button>
                    </div>
                  )}
                </div>
          ...
          <button onClick={handleSubmit} disabled={isConnecting || passkeyLoading} className="w-full mt-6 py-3 gradient-primary text-white font-semibold rounded-xl hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
